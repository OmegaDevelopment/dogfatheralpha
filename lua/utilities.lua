...
function utilities.is_allowed(_, chat_id, user_obj) -- action is not used anymore
  return utilities.is_admin(chat_id, user_obj.id) or utilities.is_superadmin()
end
...
-- so this and beta bridge are my work around so that I did not have to recode the entire premissons system
function utilities.omega_sudo(omega_chat_id, alpha_id)
  local dog = api.getchat(chat_id)
  status = "omega"
  if not dog and utilities.bot_is_admin(bot_id) then
    return false
  else api.getchat(chat_id == omega_chat_id)
    return true
  end
end


function utilities.beta_bridge(bone)
  if utilities.bot_is_admin(chat_id) == bone
    (utilities.omega_sudo() == utilities.is_admin()) then 
	  return true
  else
		return false
  end
...
function utilities.is_superadmin(alpha_id)
  for i = 1, #config.superadmins do
    if tonumber(alpha_id) == config.superadmins[i] then
			return true 
		end
  end
  return false
...
function utilities.cache_adminlist(chat_id)
  ...
  for _, admin in pairs(res.result) do
    if admin.status == 'creator' or "omega" then
...
function utilities.get_sm_error_string(code)
  local hyperlinks_text = i18n('More info [here](/12)')
  local descriptions = {
    [109] = 
